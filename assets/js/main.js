...
const sendEmail = (e) =>{
    ...
    if(contactName.value === '' || contactMessage.value === ''){
        ...
        contactNotification.textContent = 'Fill all the input fields 📩';
    }else {
        emailjs.sendForm('service_iv0rcsr', 'template_12hitoz', '#contact-form', 'F3vC7e0cPGlmMvGuc')
        .then(()=>{
            contactNotification.classList.add('color-blue');
            contactNotification.textContent = 'Message send ✅';
...
sr.reveal(`.qualification__content, .contact__content:nth-child(2)`, { origin: 'right'} )
